(function($) { "use strict";
	...
	jQuery(".form-js").submit(function () {
		...
		if (name == "") {
			jQuery("#name").after('<span class="form-description required-error">Por favor, insira o nome.</span>');
		}else {
			jQuery("#name").parent().find('.required-error').remove();
		}
		if (mail == "") {
			jQuery("#mail").after('<span class="form-description required-error">Por favor, insira o email.</span>');
		}else {
			jQuery("#mail").parent().find('.required-error').remove();
		}
		if (message == "") {
			jQuery("#message").after('<span class="form-description required-error">Por favor insira a mensagem.</span>');
		}else {
		...
		if (name != "" && mail != "" && message != "") {
			jQuery.post("contact_us.php",data,function (result) {
				if (result == "done") {
					...
					thisform.prepend("<div class='alerts contact-alert'><i class='fa fa-check-circle'></i><div><h3>Obrigado "+name+"!</h3><p> Entraremos em contato em breve. .</p></div></div>");
					jQuery("#name").val("");
					jQuery("#mail").val("");
					jQuery("#message").val("");
				}
